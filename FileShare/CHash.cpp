...
CString CHash::DoHash()
{
	...
	if (hashAlgo == CRC32)
	{
		...
		{
			crc32.StringCrc32(hashString, crc32r);
		}
		if (hashOperation == FILE_HASH)
		{
			crc32.FileCrc32Streams(hashFile, crc32r);
		}
...
CString CHash::GOSTHash()
{
	...
	if (hashOperation == FILE_HASH)
	{
		...
		fileToHash = fopen((char *)hashFile.GetBuffer(), "rb");
...
CString CHash::MD5Hash()
{
	...
	if (hashOperation == FILE_HASH)
	{
		...
		WideCharToMultiByte(CP_OEMCP,0,hashFile,-1,fn,hashFile.GetLength()*2,0,false);

		if (md5file(fn, 0, &m_md5))
		{
			for (int i = 0; i < 16; i++)
			{
				char tmp[3];
...
//
//
void CHash::SetHashFile(CString File)
{
	hashFile = File;
}
// End SetHashFile //////////////////////////////////////////////
